...
#include <TM1637.h> //include the TM1637 library from hard drive (download from (https://drive.google.com/file/d/1DN5pDko7D1-F4POXICIfd8m_1sAKBZt7/view)
...
//int pressure_sensor = A3; //currently simulated by a potentiometer, to be replaced by I2C INA219 module
...
int neutral_1_state = 0; //neutral 1 button state
int neutral_2_state = 0; //neutral 2 button state
int neutral_3_state = 0; //neutral 3 button state
...
pinMode(neutral_switch_1, INPUT_PULLUP);

pinMode(control_axis_2, INPUT);
pinMode(neutral_switch_2, INPUT_PULLUP);

pinMode(fingers_axis, INPUT);
pinMode(neutral_switch_3, INPUT_PULLUP);
...
current_mA = constrain (current_mA, 0, 30); //limit the value IN SOFTWARE from 0 to 25mA for ease of calculations

//==================convert data from float to int==================
pressure_val = current_mA*10;

//==================convert current from sensor to actual pressure value==================
pressure_actual = map(pressure_val, 0, 300, 0, 250);

//==================read values of axis is not locked==================
...
neutral_1_state = digitalRead(neutral_switch_1);
neutral_2_state = digitalRead(neutral_switch_2);
neutral_3_state = digitalRead(neutral_switch_3);
